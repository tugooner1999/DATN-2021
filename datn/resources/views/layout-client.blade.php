                    ...
                    success: function(result) {
                        ...
                        if (result.status === true) {
                            ...
                            if (result.data.type == 2) {
                                ...
                                $('#sale-off').html(new Intl.NumberFormat('en-GB').format(
                                        totalPriceInCart * result.data.value / 100) +
                                    " VNĐ")
                                $('.grand-totall-title').html("Tổng tiền " + new Intl
                                    .NumberFormat('en-GB').format(totalPriceInCart -
                                        totalPriceInCart * result.data.value / 100) +
                                    " VNĐ")
                            }
        ...
        $('.cart-btn').click(function(e) {
            ...
            $('#update-cart').click(function(e) {
                    ...
                    $.ajax({
                            ...
                            success: function(result) {
                                if (result.status === true) {
                                    ...
                                    $('.product-subtotal').each(function(item) {
                                        ...
                                        var totalPrice = new Intl.NumberFormat('en-GB')
                                            .format(result.data[proId].quantity * result
                                                .data[proId].price)
                                    ...
                                    if (typeVoucher == 2) {
                                        $('#sale-off').html(new Intl.NumberFormat('en-GB').format(
                                            totalPriceInCart * voucherValue / 100) + " VNĐ")
                                        $('.grand-totall-title').html("Tổng tiền " + new Intl
                                            .NumberFormat('en-GB').format(totalPriceInCart -
                                                totalPriceInCart * voucherValue / 100) + " VNĐ")
                                    ...
                                    $('#total-price-cart').html(new Intl.NumberFormat('en-GB')
                                        .format(totalPriceInCart) + " VNĐ")
                            ...
                            }) $('.grand-totall h5 span').html(result.totalPriceInCart + " VNĐ") Swal
                        .fire('', 'Cập nhật giỏ hàng thành công', 'success')
                ...
                $("#" + idProduct).fadeOut(1000, function() {
                        $.ajax({
                                ...
                                success: function(result) {
                                    if (result.status === true) {
                                        var totalPriceInCart = result.totalPriceInCart
                                        $('#total-price-cart').html(new Intl.NumberFormat(
                                            'en-GB').format(totalPriceInCart) + " VNĐ")
                                        if (typeVoucher == 2) {
                                            $('#sale-off').html(new Intl.NumberFormat('en-GB')
                                                .format(totalPriceInCart * voucherValue /
                                                    100) + " VNĐ")
                                            $('.grand-totall-title').html("Tổng tiền " +
                                                new Intl.NumberFormat('en-GB').format(
                                                    totalPriceInCart - totalPriceInCart *
                                                    voucherValue / 100) + " VNĐ")
                                        } else if (typeVoucher == 1) {
                                            $('#sale-off').html(new Intl.NumberFormat('en-GB')
                                                .format(voucherValue) + " VNĐ")
                                            $('.grand-totall-title').html("Tổng tiền " +
                                                new Intl.NumberFormat('en-GB').format(
                                                    totalPriceInCart - voucherValue) +
                                                " VNĐ")
                                        } else {
                                            $('.grand-totall-title').html("Tổng tiền " +
                                                new Intl.NumberFormat('en-GB').format(
                                                    totalPriceInCart) + " VNĐ")
                                        }
                        ...
                        $.ajax({
                                ...
                                success: function(result) {
                                    if (result.status === true) {
                                        sessionStorage.clear()
                                        $('head').append(
                                            `<style>.count-cart::after{ content:'${0}' !important}</style>`
                                        );
                                        $(".content-cart, .cart-page-title").empty()
                                        setTimeout(function() {
                                            $(".content-cart").append(
